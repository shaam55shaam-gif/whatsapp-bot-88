...
def home():
    return """
<html dir="rtl"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>V29.1 PRO</title>
<style>
...
.fbtn{background:#222;color:#fff;border:1px solid #555;padding:6px 12px;border-radius:15px;font-size:12px;white-space:nowrap}
...
#modal{position:fixed;inset:0;background:rgba(0,0,0,0.9);display:none;align-items:center;justify-content:center;z-index:999;padding:12px}
#modalBox{background:#1a1a1a;border:2px solid #FFD700;border-radius:16px;padding:16px;width:100%;max-width:360px}
</style>
</head><body>
<div class="h">Shaam V29.1 - اضغط على المباراة للتفاصيل</div>
...
<div id="tab2" style="display:none;padding:6px"><div style="display:flex;gap:4px;overflow:auto"><button onclick="loadStanding('eng.1')" class="fbtnA fbtn">ENG</button><button onclick="loadStanding('esp.1')" class="fbtn">ESP</button><button onclick="loadStanding('sau.1')" class="fbtn">SAU</button><button onclick="loadStanding('tur.1')" class="fbtn">TUR</button></div><div id="standing-box" style="margin-top:8px">اختر دوري...</div></div>
<div id="tab3" style="display:none;padding:8px"><div class="card">مباشر: <b id="s1">0</b></div><div class="card">قادمة: <b id="s2">0</b></div><div class="card">نشط: <b id="s4">0/18</b></div><div class="card">المجموع: <b id="s5" style="color:#FFD700;font-size:20px">0</b></div></div>
...
function formatDate(str){try{return str.substring(6,8)+"-"+str.substring(4,6)+"-"+str.substring(0,4);}catch(e){return str;}}
...
function openDetails(home,away,hLogo,aLogo,date,lg,score){
 var h='<div style="text-align:center"><div style="display:flex;justify-content:space-between;align-items:center;margin:10px 0"><img src="'+hLogo+'" style="width:60px;height:60px;background:#fff;border-radius:50%;padding:5px"><span style="font-size:24px;color:#FFD700;font-weight:900">VS</span><img src="'+aLogo+'" style="width:60px;height:60px;background:#fff;border-radius:50%;padding:5px"></div>';
 h+='<h2 style="margin:8px 0">'+home+'</h2><div style="color:#888">ضد</div><h2 style="margin:8px 0">'+away+'</h2>';
 h+='<div style="background:#FFD700;color:#000;padding:8px;border-radius:10px;margin:10px 0;font-weight:900;font-size:18px">'+score+'</div>';
 h+='<div style="background:#000;padding:10px;border-radius:10px;text-align:right;margin-top:10px"><div>📅 '+formatDate(date)+'</div><div>🏆 '+lg+'</div><div>⏰ قادمة</div></div>';
 h+='<button onclick="document.getElementById(\\'modal\\').style.display=\\'none\\'" style="margin-top:14px;width:100%;background:#FFD700;border:0;padding:12px;border-radius:12px;font-weight:900;font-size:16px">اغلاق</button></div>';
...
def api_standing(lg):
    try:
        ...
        standing=[{"pos":idx+1,"team":e.get('team',{}).get('displayName','?'),"points":int({s['name']:s['value'] for s in e.get('stats',[])}.get('points',0))} for idx,e in enumerate(entries[:12])]
