...
def home():
    return """
<!DOCTYPE html><html dir="rtl"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Shaam V26.2 SAFE</title>
<style>
...
.fbtn{background:#222;color:#fff;border:1px solid #555;padding:8px 14px;border-radius:20px;white-space:nowrap}
...
<div class="h">Shaam V26.2 SAFE - اضغط الدوري للفلترة</div>
...
<div id="tab2" style="display:none;padding:8px"><div style="display:flex;gap:5px;overflow:auto"><button onclick="loadStanding('eng.1')" class="fbtnA fbtn">ENG</button><button onclick="loadStanding('esp.1')" class="fbtn">ESP</button><button onclick="loadStanding('sau.1')" class="fbtn">SAU</button><button onclick="loadStanding('tur.1')" class="fbtn">TUR</button></div><div id="standing-box" style="margin-top:10px">اختر دوري...</div></div>
<div id="tab3" style="display:none;padding:10px"><h3 style="color:#FFD700">احصائيات</h3><div class="card"><div>مباشر: <b id="s1">0</b></div></div><div class="card"><div>قادمة: <b id="s2">0</b></div></div><div class="card"><div>نشط: <b id="s4">0/18</b></div></div><div class="card"><div>المجموع: <b id="s5" style="color:#FFD700;font-size:20px">0</b></div></div></div>
<script>
...
for(var k in L){
  var kk=safeId(k);
  grid.innerHTML+='<div class="box" id="box-'+kk+'" data-lg="'+k+'" onclick="filterLg(this.dataset.lg)"><b>'+L[k]+'</b><br><span id="cnt-'+kk+'" style="color:#888;font-size:11px">...</span></div>';
}
...
function filterLg(lg){currentFilter=lg;document.querySelectorAll('.box').forEach(function(b){b.classList.remove('boxActive');});var kk=safeId(lg);var box=document.getElementById('box-'+kk);if(box)box.classList.add('boxActive');renderMatches();}
...
function renderMatches(){
 ...
 var html='';if(allLive.length>0&&(currentFilter=='all'||currentFilter=='live')){var liveList=currentFilter=='live'?list:list.filter(function(m){return m.isLive;});if(liveList.length>0){html+='<div style="background:#300;padding:8px;text-align:center;color:#ff4444;font-weight:900">Live '+liveList.length+'</div>';html+=liveList.map(function(m){return '<div class="card live-card"><img class="team-logo" src="'+m.homeLogo+'"><div style="flex:1"><b>'+m.home+' vs '+m.away+'</b><br><span style="color:#FFD700">'+m.score+'</span> '+m.league+'</div><img class="team-logo" src="'+m.awayLogo+'"></div>';}).join('');}}
 var upList=list.filter(function(m){return!m.isLive;});if(currentFilter!='live'){if(currentFilter!='all')html+='<div style="text-align:center;color:#FFD700;padding:8px">'+currentFilter+' - '+upList.length+' مباراة</div>';html+=upList.slice(0,80).map(function(m){return '<div class="card"><img class="team-logo" src="'+m.homeLogo+'"><div style="flex:1"><b>'+m.home+' <span class="vs">vs</span> '+m.away+'</b><br><small>'+m.date+' | '+m.league+'</small></div><img class="team-logo" src="'+m.awayLogo+'"></div>';}).join('');}
...
async function fetchLeague(lg){
 ...
 done++;document.getElementById("dn").innerText=done+"/18";document.getElementById("st").innerText=done==18?"خلص! "+(allUp.length+allLive.length)+" مباراة":"Done "+done+"/18";if(done%4==0||done==18)renderMatches();
}
