...
def home():
    return """
<html dir="rtl"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>V28 GOLD</title>
<style>
...
.fbtn{background:#222;color:#fff;border:1px solid #555;padding:6px 12px;border-radius:15px;font-size:12px;cursor:pointer;white-space:nowrap}
...
.has{border-color:#0f0;background:#0a1a0a;box-shadow:0 0 8px #0f05}
.boxActive{border:2px solid #FFD700!important;background:#FFD700!important;color:#000!important}
.zero{opacity:0.5;background:#1a1a1a}
.card{background:#151515;border-right:4px solid #FFD700;border-radius:10px;margin:6px;padding:10px;display:flex;align-items:center;gap:8px;font-size:13px}
...
<div class="h">Shaam V28 GOLD - 18 دوري + تاريخ عربي</div>
...
<div style="display:flex;gap:4px;overflow:auto;padding:4px">
<button onclick="loadStanding('eng.1')" class="fbtnA fbtn">ENG</button>
<button onclick="loadStanding('esp.1')" class="fbtn">ESP</button>
<button onclick="loadStanding('sau.1')" class="fbtn">SAU</button>
<button onclick="loadStanding('egy.1')" class="fbtn">EGY</button>
<button onclick="loadStanding('tur.1')" class="fbtn">TUR</button>
</div>
<div id="standing-box" style="margin-top:8px">اختر دوري...</div>
</div>
<div id="tab3" style="display:none;padding:8px">
<div class="card"><div>🔴 مباشر: <b id="s1">0</b></div></div>
<div class="card"><div>📅 قادمة: <b id="s2">0</b></div></div>
<div class="card"><div>✅ نشط: <b id="s4">0/18</b></div></div>
<div class="card"><div>🏆 المجموع: <b id="s5" style="color:#FFD700;font-size:22px">0</b></div></div>
<div class="card"><div>💡 SAU و EGY موسم منتهي - الترتيب موجود بتبويب الترتيب</div></div>
</div>
<script>
...
function formatDate(str){
  try{
    var y=str.substring(0,4);var m=str.substring(4,6);var d=str.substring(6,8);
    var months={"01":"يناير","02":"فبراير","03":"مارس","04":"ابريل","05":"مايو","06":"يونيو","07":"يوليو","08":"اغسطس","09":"سبتمبر","10":"اكتوبر","11":"نوفمبر","12":"ديسمبر"};
    return d+" "+(months[m]||m);
  }catch(e){return str;}
}
...
function renderMatches(){
 ...
 var html='';if(allLive.length>0&&(currentFilter=='all'||currentFilter=='live')){var liveList=list.filter(function(m){return m.isLive;});if(liveList.length>0){html+='<div style="background:#300;padding:6px;text-align:center;color:red">🔴 مباشر '+liveList.length+'</div>'+liveList.map(function(m){return '<div class="card live-card"><img class="team-logo" src="'+m.homeLogo+'"><div style="flex:1"><b>'+m.home+' vs '+m.away+'</b><br><span style="color:#FFD700">'+m.score+'</span> - '+m.league+'</div><img class="team-logo" src="'+m.awayLogo+'"></div>';}).join('');}}
 var upList=list.filter(function(m){return!m.isLive;});
 if(currentFilter=='sau.1'||currentFilter=='egy.1'){
   if(upList.length==0) html+='<div style="text-align:center;padding:20px"><div style="font-size:40px">🏆</div><div style="color:#FFD700;font-weight:900;margin-top:8px">موسم '+currentFilter.toUpperCase()+' منتهي</div><div style="color:#aaa;font-size:12px;margin-top:4px">يبدأ الموسم الجديد في اغسطس</div><button onclick="loadStanding(\\''+currentFilter+'\\')" style="margin-top:10px;background:#FFD700;border:0;padding:8px 16px;border-radius:20px;font-weight:900">شوف ترتيب الموسم الماضي</button></div>';
 }
 html+=upList.slice(0,60).map(function(m){return '<div class="card"><img class="team-logo" src="'+m.homeLogo+'"><div style="flex:1"><b>'+m.home+' <span class="vs">vs</span> '+m.away+'</b><br><small>📅 '+formatDate(m.date)+' | 🏆 '+m.league+'</small></div><img class="team-logo" src="'+m.awayLogo+'"></div>';}).join('');
 if(list.length==0&&currentFilter!='sau.1'&&currentFilter!='egy.1')html='<div style="text-align:center;padding:20px;color:#888">لا يوجد مباريات</div>';
 document.getElementById("matches").innerHTML=html;
}
async function fetchLeague(lg){try{var r=await fetch("/api/league30/"+lg);var d=await r.json();var tot=d.up.length+d.live.length;var kk=safeId(lg);var el=document.getElementById("cnt-"+kk);if(tot>0){el.innerText=tot+" مباريات";document.getElementById("box-"+kk).className="box has";activeLeagues++;}else{el.innerText=d.msg||"موسم منتهي";document.getElementById("box-"+kk).className="box zero";}allUp=allUp.concat(d.up);allLive=allLive.concat(d.live);document.getElementById("lc").innerText=allLive.length;document.getElementById("uc").innerText=allUp.length;document.getElementById("tc").innerText=allUp.length+allLive.length;document.getElementById("s1").innerText=allLive.length;document.getElementById("s2").innerText=allUp.length;document.getElementById("s4").innerText=activeLeagues+"/18";document.getElementById("s5").innerText=allUp.length+allLive.length;}catch(e){}done++;document.getElementById("dn").innerText=done+"/18";document.getElementById("st").innerText=done==18?"خلص! "+(allUp.length+allLive.length)+" مباراة - اضغط على اي دوري":"يحمل "+done+"/18";if(done%3==0||done==18)renderMatches();}
async function loadStanding(lg){showTab(2);document.getElementById("standing-box").innerHTML="⏳ يحمل ترتيب "+lg+"...";try{var r=await fetch("/api/standing/"+lg);var d=await r.json();var html='<h3 style="color:#FFD700">📊 ترتيب '+lg.toUpperCase()+'</h3><table style="width:100%;border-collapse:collapse"><tr style="background:#FFD700;color:#000"><th style="padding:10px">#</th><th>الفريق</th><th>لعب</th><th>نقاط</th></tr>';d.standing.forEach(function(t){html+='<tr style="border-bottom:1px solid #333"><td style="padding:10px;text-align:center">'+t.pos+'</td><td style="padding:10px;display:flex;align-items:center;gap:6px"><img src="'+t.logo+'" style="width:20px;height:20px" onerror="this.style.display=\\'none\\'"><b>'+t.team+'</b></td><td style="text-align:center">'+(t.played||'-')+'</td><td style="text-align:center"><b style="color:#FFD700">'+t.points+'</b></td></tr>';});html+='</table>';document.getElementById("standing-box").innerHTML=html;}catch(e){document.getElementById("standing-box").innerHTML="لا يوجد بيانات";}}
    ...
    for i in range(1,61):
        ...
        try:
            ...
            for ev in r.json().get('events',[]):
                ...
                base={"home":c0['team']['displayName'],"away":c1['team']['displayName'],"homeLogo":c0['team'].get('logo',''),"awayLogo":c1['team'].get('logo',''),"league":lg,"date":d,"isLive":comp['status']['type']['state']=='in',"score":c0.get('score','0')+"-"+c1.get('score','0')}
                if base["isLive"]: live.append(base)
                else: up.append(base)
            if len(up)>=8: break
        except: continue
    msg="موسم منتهي - شوف الترتيب" if len(up
