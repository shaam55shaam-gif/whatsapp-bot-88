...
var topDiv=document.getElementById('top');for(var k in TOP){topDiv.innerHTML+='<div class=box' + ' id=b-'+k.replace(/\\./g,'-')+'><b>'+TOP[k]+'</b><br><small id=c-'+k.replace(/\\./g,'-')+'>⏳</small></div>'}
var gDiv=document.getElementById('g');for(var k in L){gDiv.innerHTML+='<div class="box '+(k=='syr.1'?'syria':'')+'" id=b-'+k.replace(/\\./g,'-')+'><b>'+L[k]+'</b><br><small id=c-'+k.replace(/\\./g,'-')+'>⏳</small></div>'}
...
function detail(i){var m=all[i];document.getElementById('modalC').innerHTML='<h2>'+(LOGOS[m.home]||'⚽')+' '+m.home+'<br>vs<br>'+m.away+' '+(LOGOS[m.away]||'⚽')+'</h2><p>⏰ '+ist(m.date)+'<br>⏳ '+cd(m.date)+'<br>🏟️ '+(m.league=='syr.1'?'الحمدانية - حلب':'')+'</p><button onclick="document.getElementById(\\'modal\\').style.display=\\'none\\'" style="background:gold;border:none;padding:10px 20px;border-radius:20px;font-weight:900">إغلاق</button>';document.getElementById('modal').style.display='flex'}
function fl(lg){return fetch('/api/league30/'+lg).then(r=>r.json()).then(d=>{var tot=d.up.length;var el=document.getElementById('c-'+lg.replace(/\\./g,'-'));if(el)el.innerText=tot+' مباريات';all=all.concat(d.up);all.sort((a,b)=>new Date(a.date)-new Date(b.date));document.getElementById('m').innerHTML=all.slice(0,100).map((m,i)=>'<div class=card '+(m.league=='syr.1'?'style="border-color:red"':'')+' onclick="detail('+i+')"><b>'+(LOGOS[m.home]||'⚽')+' '+m.home+' vs '+m.away+' '+(LOGOS[m.away]||'⚽')+'</b><br><small>⏰ '+ist(m.date)+' | ⏳ '+cd(m.date)+' | '+m.league+'</small></div>').join('');done++;document.getElementById('s').innerText='✅ '+done+'/'+total+' - '+all.length+' مباراة'}).catch(e=>{done++;})}
